...
def load_dataset(dataset_dir, batch_size, valid_batch_size=None, test_batch_size=None):
    ...
    train_data = data["x_train"][..., :2]
    data_mean = np.mean(train_data, axis=(0, 1, 2))
    data_std = np.std(train_data, axis=(0, 1, 2))
    data_std = np.where(data_std == 0, 1.0, data_std)

    data_mean = data_mean.reshape(1, 1, 1, 2)
    data_std = data_std.reshape(1, 1, 1, 2)

    print(f"标准化参数 -> Pick-Up Mean: {data_mean[0, 0, 0, 0]:.4f}, Std: {data_std[0, 0, 0, 0]:.4f}")
    print(f"标准化参数 -> Drop-Off Mean: {data_mean[0, 0, 0, 1]:.4f}, Std: {data_std[0, 0, 0, 1]:.4f}")
    ...
    for category in ["train", "val", "test"]:
        ...
        data["y_" + category][..., :2] = scaler.transform(data["y_" + category][..., :2])
    ...
    def create_torch_dataloader(x_numpy, y_numpy, batch_size, shuffle, num_workers=4, drop_last=False, seed=42):
        ...
        loader = DataLoader(
            ...
            pin_memory=True, drop_last=drop_last, generator=g if shuffle else None
        )
        return loader
