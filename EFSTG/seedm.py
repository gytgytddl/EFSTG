...
class TemporalEmbedding(nn.Module):
    ...
    def forward(self, x):
        granularity = self.time_day.shape[0]
        day_of_week_idx = (x[..., 2] * 6).round().long().clamp(0, 6)
        time_of_day_idx = (x[..., 3] * (granularity - 1)).round().long().clamp(0, granularity - 1)
...
class EFSTG(nn.Module):
    def __init__(self, device, input_dim, num_nodes, channels, out_dim=2,
                 input_len=16, output_len=16,
                 real_adj=None, granularity=288, dropout=0.2,
                 gc_gate_init=0.2, graph_alpha_init=0.1, k=15,
                 ablation_mode='Full', tcn_levels=3, **kwargs):
        super().__init__()
        ...
        self.graph_alpha = nn.Parameter(torch.tensor(graph_alpha_init))

        self.skip_conv = nn.Conv2d(in_channels=out_dim, out_channels=self.output_len * out_dim,
        ...
        self.end_conv = nn.Sequential(
            ...
            nn.SiLU(),
            nn.Conv2d(hidden_dim, self.output_len * out_dim, kernel_size=(1, 1))
        )
        self.identity_weight = nn.Parameter(torch.tensor(0.5))
        self.gc_gate = nn.Parameter(torch.tensor(gc_gate_init))

    def forward(self, x):
        B, C, N, T = x.shape
        x_target = x[:, :self.out_dim, :, :]

        if C >= 3:
            tem_emb = self.Temb(x.permute(0, 3, 2, 1))
        else:
            tem_emb = torch.zeros(B, self.channels, N, T, device=x.device)

        x_in = torch.cat([self.start_conv(x_target) + self.node_emb, tem_emb], dim=1)
        info_dict = {}
        ...
        info_dict['adp_adj'] = adp_adj.detach().cpu()

        if self.ablation_mode == 'w/o_GC':
            graph_out = 0
        else:
            if self.real_adj is not None:
                combined_adj = F.relu(adp_adj + self.graph_alpha * self.real_adj)
                combined_adj = combined_adj / (combined_adj.sum(dim=-1, keepdim=True) + 1e-6)
        ...
        if self.ablation_mode == 'w/o_Freq':
            ...
            if self.ablation_mode == 'w/o_Euler':
                h_freq, spec_adj = self.dynamic_gcn(h_local)
            else:
                h_freq, spec_adj = self.dynamic_gcn(self.euler(h_local))

            info_dict['spec_adj'] = spec_adj.detach().cpu()
            h_freq = self.ln_global(h_freq + h_local)
            gate = torch.sigmoid(self.fusion_gate(torch.cat([h_local, h_freq], dim=1)))
            info_dict['fusion_gate'] = gate.mean(dim=(2, 3)).detach().cpu()
        ...
        out_deep = out_deep.reshape(B, self.out_dim, self.output_len, N).transpose(2, 3)
        identity = x_target[:, :, :, -1:]

        if self.ablation_mode == 'w/o_Skip':
            final_out = out_deep + self.identity_weight * identity
        else:
            out_skip = self.skip_conv(x_target)
            out_skip = out_skip.reshape(B, self.out_dim, self.output_len, N).transpose(2, 3)
